...
/*   Updated: 2023/02/01 18:20:54 by nbouhali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void checkmap(t_long *longe)
{
	...
	if (longe->game.playersdiali != 1 || longe->game.exitdiali != 1 || longe->game.coinsdiali == 0)
		errors(1, longe);
	if (longe->mon.kdiali != 1 ||longe->mon.sdiali != 1 ||longe->mon.tdiali != 1)
		errors2(5,longe);
	checkwalls(longe);
	if (path(longe) == 0)
		errors(4, longe);
...
{
	if ((longe->map.kisma[y][x] == '0' || longe->map.kisma[y][x] == 'C' || longe->map.kisma[y][x] == 'P' || longe->map.kisma[y][x] == 'E') && visited[y][x] != 'V')
	{
		visited[y][x] = 'V';
